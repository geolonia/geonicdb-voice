...
API キーの認証（誰のキーか）と認可（何ができるか）は別の問題である。フロントエンドに埋め込む API キーにフルアクセス権を持たせると、漏洩した瞬間にあらゆる操作を許してしまう。書き込み専用キー、特定エンティティタイプのみ書き込み可能なキー、特定パスへのみアクセス可能なキー──こうした最小権限の原則を、コード変更なしに宣言的に表現したい。

XACML はリソース・アクション・属性をベースに認可ルールを記述できる OASIS 標準言語である。GeonicDB は XACML 3.0 をベースとした JSON ポリシーを採用しており、API キーやユーザーに細粒度のアクセス制御を付与できる。
...
クライアント側のバリデーションは UX 向上のためには有効だが、セキュリティ境界としては信頼できない。DevTools や curl から直接 API を叩けば、不正な型・形式・サイズのデータがいくらでも送られてくる。GeonicDB は受信したリクエストを必ずサーバー側で再検証することで、データベースに不正な値が入ることを防ぐ。

GeonicDB はこのサーバー側検証を 2 層構成で行う。共通部分（NGSI-LD エンベロープ）は Zod で型レベルに検証し、ユーザー定義部分（Custom Data Model のフィールド制約）は専用バリデータで検証する。
