...
- [Origin 制限（API キーのドメイン束縛）](#origin-制限api-キーのドメイン束縛)
...
| Origin 制限 | API キーに `allowedOrigins` を設定し、許可ドメイン以外からのトークン発行を拒否 |
...
## Origin 制限（API キーのドメイン束縛）

### 目的

フロントエンドに API キーを埋め込む構成では、ビルド成果物（JS バンドル）にキー文字列が平文で含まれる。Vite の `VITE_` プレフィックス変数はビルド時にインライン展開されるため、`dist/` を `grep` するだけで誰でも抽出できる。

- 抽出されたキーで攻撃者が自前の SPA（`evil.com`）を立て、正規キーで API を叩ける
- CORS は **ブラウザ側のブロック** に過ぎず、サーバーは「どこから呼ばれたか」を強制できない
- FIWARE 標準の PEP Proxy には API キーごとのオリジン束縛機能が無く、独自実装が必要

GeonicDB は API キーごとに **`allowedOrigins`** を設定し、`/auth/nonce` および `/oauth/token` の段階で **`Origin` ヘッダーをサーバー側で検証**する。許可されていないドメインからのリクエストはセッショントークン発行段階でブロックされ、漏洩した API キーが任意のドメインで再利用されるのを防ぐ。さらに `/auth/nonce` で発行する Nonce には Origin が HMAC で署名されているため、Nonce の流用も成立しない。
...
1. POST /auth/nonce
   ...
   { api_key }
                          ───────────────▶
                                              2. API キーから allowedOrigins を取得
                                                 Origin が allowedOrigins に含まれるか検証
                                                  - 含まれる → Nonce + PoW challenge 発行
                                                    （Nonce は Origin を HMAC で束縛）
                                                  - 含まれない → 403
                          ◀───────────────
3. PoW 計算
4. POST /oauth/token
   Origin: https://geolonia.github.io
   { grant_type: "api_key", api_key, nonce, proof }
                          ───────────────▶
                                              5. Origin を再検証
                                                 + Nonce の HMAC 検証（Origin バインド）
                                                 + PoW 検証
                                                  → セッション JWT 発行
                          ◀───────────────
```

ブラウザは `Origin` ヘッダーを JavaScript から書き換えできない（[Fetch 仕様](https://fetch.spec.whatwg.org/#http-origin) で User Agent が制御）ため、別オリジンから正規 API キーで呼んでも `Origin: evil.com` となり、サーバー側検証で拒否される。Nonce にも Origin が署名込みで含まれるため、別オリジンで取得した Nonce を流用してトークン交換することもできない。

### このデモでの使い方

1. **API キー作成時** に `--origins` で許可するオリジンをカンマ区切りで指定する（[セットアップ手順 5](#5-api-キーの作成)）
   ```bash
   geonic me api-keys create \
     --origins http://localhost:8080,https://geolonia.github.io \
     ...
   ```
2. **後から変更** する場合は `update` サブコマンドを使う
   ```bash
   geonic me api-keys update <keyId> --origins https://geolonia.github.io
   ```
3. **環境ごとに別キー** を発行することを推奨する。1 つのキーに dev / staging / prod のオリジンを混ぜると、いずれかの環境で漏れた際の影響範囲が広がる
4. **オリジンの記述形式** は `<scheme>://<host>[:<port>]`。パス・末尾スラッシュは含めない（例: `https://geolonia.github.io` であって `https://geolonia.github.io/` ではない）

### 効果

| 攻撃シナリオ | Origin 制限なし | Origin 制限あり（このデモ） |
|--------------|------------------|---------------------------|
| バンドルから抽出したキーで `evil.com` の SPA から呼び出し | 許可 | `/auth/nonce` でトークン発行段階で拒否 |
| 開発者ツールでキーをコピーし別ホストの自前 SPA で利用 | 許可 | 同上 |
| 別オリジンで取得した Nonce を正規オリジンへ流用 | 該当なし | Nonce の HMAC に Origin が含まれるため拒否 |
| `curl --header 'Origin: <許可済み>'` でサーバー直叩き | 許可 | `Origin` ヘッダーは spoof 可能なため通過する。レート制限と最小権限ポリシーで影響を限定する必要あり |

> **限界**: `Origin` ヘッダーはブラウザ環境では信頼できるが、`curl` 等の任意クライアントからは自由に設定できる。Origin 制限は「ブラウザ経由の不正流用」を遮断する層であり、サーバー側攻撃まではカバーしない。**XACML による最小権限**と**API キーごとのレート制限**（`--rate-limit`）を併用して、多層防御として機能させる。
...
複数テナントに所属しているアカウントでは、`auth login` にテナントを指定する。指定しない場合、対話環境なら候補が一覧表示されて選択を求められるが、非対話環境（CI 等）ではエラー終了する。

```bash
geonic auth login --tenant <テナント名 or ID>
```

ログイン後、後の手順 5 で `.env` に設定する**テナント名**を控えておく。`geonic me` の出力に含まれる `tenantName` がそれにあたる。
...
|--------|------|
| `--name` | キーの識別名 |
| `--origins` | 許可するオリジンをカンマ区切りで指定（[Origin 制限](#origin-制限api-キーのドメイン束縛)）。`<scheme>://<host>[:<port>]` 形式、パス・末尾スラッシュは含めない |
| `--policy` | バインドする XACML ポリシー（[XACML](#xacmlextensible-access-control-markup-language)） |
| `--dpop-required` | リクエストに DPoP Proof の添付を必須化（[DPoP](#dpopdemonstrating-proof-of-possession)） |
| `--rate-limit` | キー単位のレート制限（requests / minute） |

オリジンを後から変更したい場合は `update` を使う。
...
| `VITE_GEONICDB_TENANT` | 手順 2 で控えたテナント名（`geonic me` の `tenantName`）。UUID の `tenantId` ではない |
